...
The simplest way to explain the concepts of the package is through an example. `joineRML` comes with the data set `heart.valve`. This data set is in so-called *long* or *unbalanced* format:
...
fit.se <- bootSE(fit, nboot = 25, use.mle = TRUE, control = list(
    nMCmax = 10000, earlyPhase = 20, mcmaxIter = 50, 
    convCrit = "either", tol0 = 1e-02, tol2 = 1e-02),
  progress = FALSE)
```
...
Note that in the interests of compiling this vignette in a minimum amount of time, we have limited `bootSE` to only 25 iterations. In general, users will want at least 100-200 iterations. Also, we have increased the convergence tolerance limits to $10^{-2}$. In general, stricter criteria will be required for reliable standard error estimates; the default is $5 \times 10^{-3}$.
...
* `joineR` requires a data pre-processing step in order to generate a `joint.data` object, whereas `joineRML` can work straight from the data frame.
...
fit.joinerml.boot <- bootSE(fit.joinerml, nboot = 25, use.mle = TRUE, control = 
                              list(earlyPhase = 25, convCrit = "either", 
