...
Tidiers for objects of class `mjoint` have been included in latest release of the `broom` package (version 0.5.0). 

As the introductory vignette to `broom` outlines, 
...
fit <- mjoint(
  ...
  data = list(hvd, hvd),
  inits = list("gamma" = c(0.11, 1.51, 0.80), "beta" = c(2.51, -0.01, 0.14, 0.19, 5.09, -0.01, -0.20), "sigma2" = c(0.36, 0.04)),
  timeVar = "time")
```
...
tidy(fit, conf.int = TRUE, conf.level = 0.99)
```
...
```{r tidy-boot, cache = TRUE}
bSE <- bootSE(fit, nboot = 5, safe.boot = TRUE, progress = FALSE)
...
```{r augment-nd}
preds = augment(fit)
head(select(preds, num, log.grad, log.lvmi, starts_with(".fitted"), starts_with(".resid")))
```
...
```{r augment-plot}
preds %>% 
  filter(num %in% c(26, 36, 227, 244)) %>% 
  ggplot(aes(x = time, colour = num)) +
...
This allows comparing competing models easily. Say for instance that we fit a second model with a random slope of time for the longitudinal submodel of aortic gradient (compared to random intercept only in the previous model):

```{r glance-fit2, cache = TRUE}
set.seed(67890)
fit2 <- mjoint(
  formLongFixed = list("grad" = log.grad ~ time + sex + hs, 
                       "lvmi" = log.lvmi ~ time + sex),
  formLongRandom = list("grad" = ~ time | num,
                        "lvmi" = ~ time | num),
  formSurv = Surv(fuyrs, status) ~ age,
  data = list(hvd, hvd),
  inits = list("gamma" = c(0.11, 1.51, 0.80), "beta" = c(2.51, -0.01, 0.14, 0.19, 5.09, -0.01, -0.20), "sigma2" = c(0.36, 0.04)),
...
```{r glance-comparison}
bind_rows(
  mutate(glance(fit), model = "fit"),
  mutate(glance(fit2), model = "fit2")
)
```
